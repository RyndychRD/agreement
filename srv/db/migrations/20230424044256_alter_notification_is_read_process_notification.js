...
exports.up = async function (knex) {
  ...
  return knex.raw(
    "ALTER TABLE public.notification_is_read ADD CONSTRAINT notification_is_read_notification_type_check CHECK ((notification_type = ANY (ARRAY['ReworkDocument'::text, 'Signing'::text, 'IncomeTask'::text, 'CompleteTask'::text, 'OnRegistration'::text, 'Approved'::text, 'Completed'::text, 'Rejected'::text, 'SignedOOPZ'::text, 'ProcessingDocument'::text])));"
  );
};
